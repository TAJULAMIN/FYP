...
export default function BookTable() {
  ...
  useEffect(() => {
    const fetchAvailableTables = async () => {
      if (!formData.date || !formData.time) {
        setAvailableTables([]); // ✅ clear when no date/time
        return;
      }
      try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ date: "", time: "", guests: "", branch: "Swabi", tableNumber: "" });
      setErrors({});
      ...
      <BackgroundBox>
        <FormContainer component="form" noValidate autoComplete="off" onSubmit={handleSubmit}>
          ...
          <Grid container spacing={2}>
            ...
            {/* ✅ Only show Select Table if date & time are chosen */}
            {formData.date && formData.time && (
              <Grid item xs={12}>
                <TextField
                  select
